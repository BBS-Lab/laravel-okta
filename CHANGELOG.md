...
- Derive the OIDC `redirect_uri` from the panel's `okta/callback` route, so `OKTA_REDIRECT_URI` is optional (an explicit `services.{driver}.redirect` still wins).
...
- `redirect()` now fails gracefully (flash + back to login) instead of a 500 when the Socialite driver cannot be built.
